...
from src.benchmark import total_variation_loss, tikhonov_loss, joint_tikhonov_tv_loss
...
class Regularization_method:
    ...
    Supported types: l2 (Tikhonov), tv (total variation), joint_tikhonov_tv, l2tv, None.
    """
    def __init__(self, regularization_type):
        ...
        Args:
            regularization_type: Regularization type ('l2'/'tv'/'joint_tikhonov_tv'/None)
        """
    ...
    def initialize_regularization(self):
        ...
        elif self.regularization_type == 'joint_tikhonov_tv':
            regularization = joint_tikhonov_tv_loss
        else:
            regularization = None
    ...
    def get_reg_loss(self, mu):
        ...
        elif self.regularization_type == 'l2tv':
            reg_loss = joint_tikhonov_tv_loss(mu)
        else:
...
class run_inversion:
    ...
    def sample(self, mu, mu_true, y, ts, lr=0.001, reg_lambda=0.01, fwi_forward=None,
        ...
        """
        Execute the resistivity model inversion optimization main loop.
        ...
        Args:
            ...
            lr: Optimizer learning rate (default 0.001)
        ...
        # Override regularization type if specified at call time
        ...
        optimizer = torch.optim.AdamW([mu], lr=lr, betas=(0.9, 0.999), weight_decay=1e-2)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=ts, eta_min=0)

        results_dict = ResultsDict(self.data_trans, self.ssim_loss, loss_type, self.regularization_method, reg_lambda)

        # MT data preprocessing: simulate real-world noise and missing traces
        ...
        for l in pbar:
            # 1. MT forward: generate predicted MT data
